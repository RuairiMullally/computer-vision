...
void MyApplication()
{
	...
	for (int video_file_no = 1; (video_file_no <= number_of_videos); video_file_no++)
	{
		...
		// At this point the video file is open and we can process it
		{
			...
			cv::Mat consec_count(current_frame.rows, current_frame.cols, CV_16U, cv::Scalar(0));
			cv::Mat persistentMask(current_frame.rows, current_frame.cols, CV_8U, cv::Scalar(0));
			...
			while (!current_frame.empty())
			{
				...
				absdiff(current_frame, background_frame, difference_frame);
				...
				threshold(difference_frame, difference_frame, 30, 255, THRESH_BINARY);
				// perform opening and closing to remove noise and fill in holes
				morphologyEx(difference_frame, difference_frame, MORPH_OPEN, getStructuringElement(MORPH_RECT, Size(3, 3)));
				morphologyEx(difference_frame, difference_frame, MORPH_CLOSE, getStructuringElement(MORPH_RECT, Size(12, 12)));
				...
				writeText(difference_frame, frame_title, 15, 15);
				imshow(string("Difference - ") + abandoned_removed_video_files[video_file_no-1], difference_frame);
